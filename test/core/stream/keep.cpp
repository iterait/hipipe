...
BOOST_AUTO_TEST_CASE(test_keep_unique_column)
{
    ...
    batch1.insert_or_assign<Unique3>();
    batch1.extract<Unique3>().push_back(std::make_unique<int>(5));
    data.push_back(std::move(batch1));
    batch2.insert_or_assign<Unique>();
    batch2.extract<Unique>().push_back(std::make_unique<int>(2));
    batch2.insert_or_assign<Unique3>();
    batch2.extract<Unique3>().push_back(std::make_unique<int>(6));
    data.push_back(std::move(batch2));
    ...
    BOOST_TEST(!stream.at(0).contains<Unique3>());
    ...
    BOOST_TEST(!stream.at(1).contains<Unique3>());
