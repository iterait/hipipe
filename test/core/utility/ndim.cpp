...
BOOST_AUTO_TEST_CASE(test_flatten)
{
    ...
    test_ranges_equal(flat_view(vec), rgv::iota(1, 16));
}
...
BOOST_AUTO_TEST_CASE(test_flatten_cutoff_dim1)
{
    ...
    auto retrieved =
      rg::to<std::vector<std::vector<std::vector<int>>>>(flat_view<1>(vec));
...
BOOST_AUTO_TEST_CASE(test_flatten_cutoff_dim2)
{
    ...
    auto retrieved = rg::to<std::vector<std::list<int>>>(flat_view<2>(vec));
...
BOOST_AUTO_TEST_CASE(test_flatten_move_only)
{
    ...
    test_ranges_equal(flat_view(vec) | rgv::indirect, rgv::iota(1, 5));
}
...
BOOST_AUTO_TEST_CASE(test_reshape_1d)
{
    ...
    std::vector<int> rvec = rg::to_vector(reshaped_view<1>(vec, {15}));
...
BOOST_AUTO_TEST_CASE(test_reshape_2d)
{
    ...
    auto rvec =
      rg::to<std::vector<std::vector<int>>>(reshaped_view<2>(vec, {3, 5}));
...
BOOST_AUTO_TEST_CASE(test_reshape_3d)
{
    ...
    auto rvec =
      rg::to<std::vector<std::vector<std::vector<int>>>>(reshaped_view<3>(vec, {3, 1, 5}));
    BOOST_TEST(rvec.size() == 3);
    for (std::size_t i = 0; i < 3; ++i) {
...
BOOST_AUTO_TEST_CASE(test_reshape_auto_dimension)
{
    ...
    auto rvec1 =
      rg::to<std::vector<std::vector<int>>>(reshaped_view<2>(vec, {3, -1}));
    auto rvec2 =
      rg::to<std::vector<std::vector<int>>>(reshaped_view<2>(vec, {-1, 5}));
...
BOOST_AUTO_TEST_CASE(test_random_fill_2d)
{
    ...
    auto check = [](auto vec, std::vector<long> unique, long unique_total) {
        ...
        std::vector<double> all_vals = rg::to_vector(flat_view(vec));
        all_vals |= rga::sort;
